...
def load_movies():
    ...
    # 영화 정보를 사전 형태로 로드
    cursor.execute("SELECT title, genre, mood, length FROM movies")
    movies = {row[0]: {"genre": str(row[1]), "mood": str(row[2]), "length": str(row[3])} for row in cursor.fetchall()}

    conn.close()
    return movies

def save_movie(title, genre, mood, length):
    # SQLite 데이터베이스 연결
    conn = sqlite3.connect('movies.sqlite')
    cursor = conn.cursor()
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?)
    ''', (title, int(genre), int(mood), int(length)))
...
def movie_chatbot():
    ...
    while True:
        ...
        if action == '1':
            ...
            # 영화 정보 추가
            save_movie(title, genre, mood, length)
